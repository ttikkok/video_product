...
> 如果你未遇到编译错误，可以忽略下段内容
- 自`2.0.0`开始，本组件用到了`TypeScript`你可遇到出现编译错误，如下:
```
...
```
- 可以尝试如下解决办法：
1. 如果你是`cli`项目，尝试安装`@vue/cli-plugin-typescript`和`typescript`，执行`npm install typescript @vue/cli-plugin-typescript` 或 `yarn add typescript @vue/cli-plugin-typescript`，同时在根目录创建`tsconfig.json`，并进行个性化配置，[查看推荐配置](https://uniapp.dcloud.net.cn/tutorial/typescript-subject.html#%E4%BF%AE%E6%94%B9-typescript-%E9%85%8D%E7%BD%AE)：
更多说明请查看官方教程[TypeScript 支持](https://uniapp.dcloud.net.cn/tutorial/typescript-subject.html)
2. 非`cli`项目，请更新`HBuilder X`，或者官方的TS版本过低导致的问题请到`\HBuilderX\plugins\compile-typescript`目录下，执行`npm install typescript -D`或`yarn add typescript -D`
...
| handelSeek `1.1.8`		|		| 跳转到指定位置，单位 s(需要配合`seekTime`使用)	|	~~`1.1.0版本`之前seek ~~		|
...
- 如果你遇到编译失败的错误，请查看[vue2使用必看](#jump2)尝试解决
