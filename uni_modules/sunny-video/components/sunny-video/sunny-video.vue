<template>
	<view class="sunny-video-wrapper">
		<view v-if="showTopPlaceholder" class="top-placeholder" :style="{ height: addUnit(topPlaceholderHeight) }"></view>
		<video
			class="sunny-video"
			:id="videoId"
			:title="title"
	...
	>
		...
		<cover-view v-if="state.visibleTrialEndBox" class="trialEndBox" :style="[trialEndBoxStyle]">
			...
			<cover-view class="closeBtn" @click.stop="handleClickTrialEnd">
				{{ btnText }}
			</cover-view>
			<!-- #endif -->
		</cover-view>
		...
		<cover-view class="mplayer-toast" :class="{ 'mplayer-show': state.showMplayerToast }">
			<!-- #ifdef H5 -->
			<text class="text">
				记忆你上次看到{{ convertSecondsToHMS(seekTime) }}
				<text class="btn_text" @click="handelSeek">跳转</text>
			</text>
			<!-- #endif -->
			<!-- #ifndef H5 | APP-NVUE -->
