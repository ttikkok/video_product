<template>
  ...
  >
    ...
    <view
      ...
      :key="index"
    >
      <u-icon :name="item.icon" :color="current === index ? activeColor : inactiveColor" size="24"></u-icon>
      <text class="u-subsection__item__text" :style="[textStyle(index)]">{{
        $t(getText(item))
      }}</text>
      <u-badge class="ml-1" v-if="item.total > 0" :value="total" bgColor="#F53F3F"></u-badge>
    </view>
  </view>
</template>
 ...
 * @property {String}			activeColor		激活时的颜色（默认 '#6666ff' ）
